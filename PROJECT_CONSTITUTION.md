...
*   **Multi-Format Data Upload System**: Comprehensive file upload system supporting both CSV and COCO JSON formats with intelligent format detection:
    *   **CSV Format Support**: `multer` and `csv-parser` pipeline for traditional CSV uploads with columns: `filename`, `url`, `width`, `height`, `prompt`, and optional `img_key`
    *   **COCO JSON Format Support**: Complete COCO dataset format implementation following standard COCO structure with `images` and `annotations` arrays
    *   **Automatic Format Detection**: Smart format detection by analyzing file content structure (JSON with images/annotations arrays = COCO, otherwise CSV)
    *   **Format Enforcement**: Prevents mixing formats in a single dataset - once format is determined by first upload, subsequent uploads must match
    *   **Intelligent URL Construction**: COCO parser automatically appends filename to base URLs when needed (e.g., URLs ending with `/` or without file extension)
    *   **Multiple Captions Support**: COCO datasets can have multiple captions per image - first caption becomes primary `prompt`, additional captions stored in `additionalCaptions` array
    *   **License Information**: COCO format preserves license data from both images and global licenses array
    *   **Adaptive UI**: Interface automatically adapts to dataset format - shows appropriate upload instructions, file type filters (.csv/.json), and format-specific fields in Data Studio
...
*   **Comprehensive File Management System**: Complete CSV file storage and retrieval architecture with visual version organization:
    ...
    *   **Smart File Versioning**: Complete version management system with intelligent UI organization:
        *   **Chronological Sorting**: Files automatically sorted by upload date (DESC) showing newest versions first
        *   **Latest Version Highlighting**: Most recent version prominently displayed with primary color theme, distinctive border, and "Latest Version" badge
        *   **Archive Section**: Previous versions visually separated in dedicated "Archive Versions" section with muted appearance
        *   **Detailed Timestamps**: Full date and time display (date + hours:minutes:seconds) for precise version tracking
        *   **Visual Hierarchy**: Clear distinction between current and archived files through color coding, opacity, and layout separation
        *   **Responsive Actions**: Latest version features primary action button, archives use outline variant
    *   **API Endpoints**: Dedicated endpoints for file listing (`/files`) and downloading (`/files/:id/download`) with automatic version sorting
    *   **Complete Localization**: All version-related UI elements fully translated in English and Russian
*   **Dataset Export System**: Professional export functionality for training framework integration:
    *   **Backend Export Infrastructure**: Robust export system with dedicated utilities and API endpoints:
        *   **Export Utility**: Specialized helper functions (`backend/src/utils/exportHelper.ts`) for URL-to-path conversion and format generation
        *   **Smart URL Processing**: Intelligent extraction of relative file paths from full URLs, removing domain and protocol while preserving directory structure
        *   **REST API Endpoint**: Dedicated export endpoint (`GET /api/datasets/:id/export/kohya`) with proper authentication and access control
        *   **JSONL Generation**: Accurate JSON Lines format generation with proper line-by-line structure for kohya_ss/sd-scripts compatibility
    *   **Kohya SS / sd-scripts Format Support**: Complete implementation of JSONL format for popular training framework:
        *   **Standard Format Compliance**: Each line contains JSON object with `file_name` (relative path) and `caption` (prompt text) fields
        *   **Path Normalization**: Automatic conversion of URLs like `https://datasets.pbc.red/media/datasets/games/poker/image.png` to `media/datasets/games/poker/image.png`
        *   **Training Framework Integration**: Export format designed for direct consumption by kohya_ss train_network.py scripts
    *   **Frontend Export Interface**: Modern, user-friendly export dialog with comprehensive options:
        *   **ExportDialog Component**: Professional modal dialog (`src/components/ExportDialog.tsx`) with format selection and export configuration
        *   **Format Selection UI**: Radio button interface for choosing export format with descriptive labels and format examples
        *   **Visual Format Preview**: Code snippet preview showing exact structure of exported data for user reference
        *   **Information Notes**: Clear explanation that users need to download images separately and place them in corresponding directories
        *   **Export Progress**: Loading states with spinner animation during export operation
        *   **Error Handling**: Comprehensive error handling with user-friendly messages for various failure scenarios (no images, access denied, etc.)
    *   **Files and Versions Tab Integration**: Seamless integration into existing dataset management interface:
        *   **Export Section**: Dedicated "Export" card in Files and Versions tab positioned below Dataset Files section
        *   **Descriptive UI**: Clear title, description, and informational text explaining export functionality
        *   **One-Click Export**: Simple button to open export dialog without complex prerequisites
        *   **Backend Validation**: Server-side validation ensures dataset has images before allowing export
    *   **Complete Localization**: Full internationalization support for all export features:
        *   **English Translations**: Complete translation set in `src/locales/en/pages.json` covering all export UI elements
        *   **Russian Translations**: Full Russian localization in `src/locales/ru/pages.json` for all export-related text
        *   **Toast Notifications**: Localized success/error messages for export operations
    *   **Security and Access Control**: Proper permission handling throughout export workflow:
        *   **Authentication Check**: Export endpoint uses `checkJwtOptional` middleware respecting dataset visibility rules
        *   **Owner and Admin Access**: Private datasets can only be exported by owner or administrators
        *   **Public Dataset Access**: Public datasets exportable by any authenticated user
...
### 5.10. Markdown Rich Text System (Fully Implemented)

*   **Industry-Standard Markdown Support**: Complete implementation of GitHub Flavored Markdown (GFM) for dataset descriptions and discussion posts:
    *   **Supported Syntax**: Bold (`**text**`), italic (`*text*`), strikethrough (`~~text~~`), links (`[text](url)`), images (`![alt](url)`), code blocks with syntax highlighting, inline code, blockquotes, ordered/unordered lists, tables, headings (H1-H6), horizontal rules, task lists
    *   **Smart Image Handling**: External images render as clickable thumbnails with max-height constraint, click to open full-size modal dialog (similar to Data Studio image viewer), lazy loading for performance, automatic border and hover effects
    *   **Safe Link Rendering**: All external links open in new tab with `rel="noopener noreferrer"` security attributes, automatic `target="_blank"` for user safety
    *   **XSS Protection**: Enterprise-grade security through `rehype-sanitize` library blocking all script tags, event handlers (`onclick`, `onerror`), JavaScript URLs (`javascript:`), dangerous tags (`<iframe>`, `<object>`, `<embed>`), style injection, ensuring 100% safe user-generated content

*   **Dataset Description Editor**: Professional Markdown editing experience for dataset authors:
    *   **Visual Editor Component**: Powered by `@uiw/react-md-editor` with split-view interface showing Edit and Preview tabs, live rendering of Markdown as user types, toolbar with common formatting buttons
    *   **Access Control**: Edit button visible only to dataset owners and administrators, permission checks on both frontend (UI visibility) and backend (API validation)
    *   **Modal Dialog Interface**: Full-screen editing experience in responsive dialog, tabbed navigation between editing and preview, save/cancel actions with loading states
    *   **API Integration**: Dedicated endpoint (`PATCH /api/datasets/:id/description`) for updating descriptions, stores both Markdown and plain text versions for backward compatibility

*   **Discussion System Markdown Integration**: Rich text support in all discussion posts:
    *   **PostEditor Enhancement**: Textarea-based editor with Markdown syntax help popover, inline documentation showing formatting examples (bold, italic, links, images, code), maintained reply/quote functionality with Markdown support
    *   **Post Rendering**: MarkdownRenderer component displays formatted content in all posts, automatic fallback to plain text for legacy posts without Markdown
    *   **Edit History**: PostEditHistory component shows Markdown diffs, preserved formatting in revision comparisons, DiffViewer adapted for Markdown content
    *   **Create/Reply/Edit Flow**: All discussion creation and editing flows support Markdown, API endpoints updated to accept `contentMarkdown` parameter, history tracking includes both plain and Markdown versions

*   **Technical Stack & Security**:
    *   **Libraries**: `react-markdown` (rendering), `@uiw/react-md-editor` (editing), `remark-gfm` (GitHub Flavored Markdown), `rehype-sanitize` (HTML sanitization)
    *   **Database Schema**: New fields `descriptionMarkdown` (datasets), `contentMarkdown` (discussion_posts), `oldContentMarkdown`/`newContentMarkdown` (discussion_edit_history)
    *   **Migrations**: `1758200000000-AddMarkdownDescriptionToDatasets`, `1758200100000-AddMarkdownToDiscussionPosts`
    *   **Security Testing**: Comprehensive XSS protection verified against 10 attack vectors documented in SECURITY_TEST_MARKDOWN.md, all tests passing with rehype-sanitize blocking malicious content
    *   **TypeScript Types**: Full type safety with optional Markdown fields in Dataset, DiscussionPost, DiscussionEditHistory interfaces

*   **Styling & UX**:
    *   **Custom CSS**: Dedicated stylesheet (`src/styles/markdown.css`) with responsive typography, proper spacing for all Markdown elements, dark mode support via CSS variables
    *   **Component Styling**: Tables with borders and hover effects, code blocks with monospace font and background, blockquotes with left border accent, lists with proper indentation, headings with visual hierarchy
    *   **Image Modal**: Reusable MarkdownImageModal component for full-size image viewing, max-width/max-height constraints for viewport fitting, click outside to close, smooth transitions

*   **Internationalization**: Complete i18n coverage in English and Russian:
    *   **Editor UI**: Tab labels ("Edit", "Preview"), placeholders with Markdown examples, button labels ("Save", "Cancel")
    *   **Help Text**: Markdown syntax help popover fully translated, formatting examples in user's language
    *   **Toast Messages**: Success/error notifications for description updates, all user-facing strings localized
    *   **Translation Keys**: Added `markdown.*` namespace in common.json, dataset-specific keys in pages.json

*   **Smart Preview System** (Dataset Cards Enhancement):
    *   **Automatic Image Extraction**: Intelligent extraction of first image from Markdown descriptions using regex pattern `/!\[([^\]]*)\]\(([^)]+)\)/g`
    *   **Image Validation**: Custom React hook (`useImageDimensions`) for validating image dimensions, only displaying images wider than 300px to maintain professional card appearance
    *   **Text Extraction**: Sophisticated text processing utility (`extractFirstParagraph`) that strips all Markdown syntax (images, links, headers, bold, italic, code blocks, lists) and extracts clean first paragraph for preview
    *   **Visual Hierarchy**: Dataset cards display featured image (if valid) at top, followed by clean text preview, creating engaging visual presentation
    *   **Performance Optimization**: Uses `useMemo` hooks to prevent unnecessary recomputation of image/text extraction, Image loading with Intersection Observer pattern
    *   **State Synchronization**: Fixed description update bug with proper parent-to-child state propagation through callback chain (Dataset page → DatasetTabs → DatasetCardTab)
    *   **Consistent UI**: Applied to all dataset listing pages (Index, AllDatasets, User profile) for unified user experience
    *   **Utility Functions**: Centralized Markdown processing utilities in `src/lib/utils.ts`:
        *   `extractMarkdownImages(markdown)`: Extracts all images with alt text and URLs
        *   `extractFirstParagraph(markdown, maxLength)`: Cleans Markdown and returns plain text preview

*   **Features NOT Yet Implemented** (Future Enhancements):
    *   **Real-time Updates**: No WebSocket or polling for live discussion updates
    *   **Advanced Markdown Editor**: No WYSIWYG mode, no drag-and-drop image uploads, no Markdown templates
...
*Latest Update: January 2026 - Markdown Smart Preview System Enhancement: Implemented intelligent preview system for dataset cards that automatically extracts and displays images and text from Markdown descriptions. Created sophisticated utility functions (`extractMarkdownImages`, `extractFirstParagraph`) in `src/lib/utils.ts` that parse Markdown content to extract first image and clean text preview by stripping all syntax (headers, bold, italic, links, code blocks, lists). Developed custom React hook `useImageDimensions` in `src/hooks/use-image-dimensions.ts` for validating image dimensions, ensuring only images wider than 300px are displayed as featured images on dataset cards for professional appearance. Enhanced `DatasetListItem` component with visual hierarchy: featured image at top (if valid), followed by clean text preview, creating engaging card presentation across all listing pages (Index, AllDatasets, User profile). Fixed critical state synchronization bug where dataset description changes weren't reflected in UI after editing - implemented proper callback propagation chain (Dataset page → DatasetTabs → DatasetCardTab → DatasetDescriptionEditor) with `onDescriptionUpdate` callback to ensure parent components update their state when description is saved. Performance optimized with `useMemo` hooks preventing unnecessary recomputation of Markdown parsing. System now provides HuggingFace-style visual dataset cards with automatic image extraction and clean text previews, significantly improving dataset discovery and user engagement.*

*January 2026 - Markdown Rich Text System Implementation: Deployed comprehensive Markdown support system for dataset descriptions and discussion posts, transforming plain text into rich formatted content with HuggingFace-style presentation. Implemented complete backend infrastructure with database migrations adding `descriptionMarkdown` to datasets table and `contentMarkdown` + `oldContentMarkdown`/`newContentMarkdown` to discussion posts and edit history tables. Backend API endpoints updated to accept and store Markdown content (`PATCH /api/datasets/:id/description`, discussion post creation/editing endpoints). Frontend features include professional MarkdownRenderer component with GitHub Flavored Markdown support (tables, strikethrough, task lists), automatic HTML sanitization via rehype-sanitize preventing XSS attacks, custom image rendering with click-to-enlarge modals similar to Data Studio, safe link handling with `rel="noopener noreferrer"`, and comprehensive styling for code blocks, blockquotes, lists, and tables. Implemented DatasetDescriptionEditor with visual Markdown editor using @uiw/react-md-editor library, split-view editing with live preview tab, toolbar with formatting buttons, and Edit button integration in DatasetCardTab visible only to owners and administrators. Discussion system enhanced with Markdown support in PostEditor component maintaining reply/quote functionality, inline Markdown help popover with syntax examples, updated DiscussionPostComponent for rich content rendering, and PostEditHistory displaying Markdown diffs. Added dedicated CSS stylesheet (`src/styles/markdown.css`) with responsive design, dark mode support, and professional typography. Complete internationalization in English and Russian covering all UI elements (editor tabs, placeholders, buttons, help text, toast messages). Security testing documented in SECURITY_TEST_MARKDOWN.md covering 10 XSS attack vectors with all tests passing. TypeScript types updated across Dataset, DiscussionPost, and DiscussionEditHistory interfaces with optional Markdown fields maintaining backward compatibility. System provides HuggingFace-level rich text editing capabilities while maintaining enterprise-grade security through proven sanitization libraries.*

*October 2025 - Activity Tracking System Enhancement: Expanded Recent Changes and User Edit History systems with comprehensive dataset activity tracking. Implemented new `DatasetActivity` entity for logging dataset creation and file upload events with dedicated migration (`1758100000000-CreateDatasetActivity`). Enhanced backend endpoints (`POST /api/datasets` and `POST /api/datasets/:id/upload`) to automatically log `dataset_created` and `file_uploaded` activities. Extended Recent Changes API (`GET /api/recent-changes`) and User Edits API (`GET /api/users/:id/edits`) to aggregate and display six activity types: caption edits, discussion creation/posts, post edits, dataset creation, and file uploads. Updated frontend components (RecentChanges page and UserEditsTab) with comprehensive UI for displaying new activity types including visual icons (FolderPlus, Upload), clickable dataset links, file metadata (filename, image count), and proper timestamp formatting. Added complete internationalization in English and Russian for all new activity type labels (`createdDataset`, `uploadedFile`, `viewDataset`, `imagesUploaded`). System now provides complete transparency of all user actions in the platform, from dataset management to content contributions. Updated PROJECT_CONSTITUTION.md to document expanded activity tracking architecture with detailed descriptions of all six activity types and their respective data structures.*
