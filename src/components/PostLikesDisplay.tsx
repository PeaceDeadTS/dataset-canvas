...
export const PostLikesDisplay: React.FC<PostLikesDisplayProps> = ({
  likes,
  isLiked,
  isLoading,
  onToggleLike,
          ...
          >
            <div className="flex -space-x-1.5">
              {displayedLikes.map((like, index) => (
                <Avatar
                  key={like.id}
                  className="h-6 w-6 border-2 border-background hover:z-10 transition-transform hover:scale-110"
              ...
              {hasMoreLikes && (
                <div
                  className="h-6 w-6 rounded-full border-2 border-background bg-muted flex items-center justify-center text-[10px] font-medium"
                  style={{ zIndex: 0 }}
                >
                  +{remainingCount}
                </div>
